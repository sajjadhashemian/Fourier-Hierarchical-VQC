"""Deterministic hierarchical continuation trainer."""
    ...
    def run(
        ...
        *,
        ...
        for si, st in enumerate(schedule.stages):
            ...
            new_params = sorted(set(active) - prev_active)
            ...
            for t in range(st.steps):
                ...
                g_act = sm.grad(theta, indices=active, seed=int(rng.integers(1 << 30)))
                theta[active] -= lr_t * g_act
                if self.torus:
                ...
                hist["grad_norm_active"].append(float(np.linalg.norm(g_act)))
                hist["method"].append("dhfc")
                it += 1

            prev_active = set(active)

        return DHFCResult(theta=theta, history=hist, meta=meta)
